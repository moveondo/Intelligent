    ...
    <div class="list-wrap">
        <div class="nav">
            <el-tabs v-model="tabSelect" @tab-click="queryTarget">
                <el-tab-pane :label="$t('analyst.industryTracking')"></el-tab-pane>
        ...
        <ul class="target-list" v-loading="loading">
            ...
            <li class="target-content" v-for="item in targetList" :key="item.index">
                <div class="target-corp-content">
                    <p class="target-corp">{{ item.bizName }} <span>{{item.bizCode }}</span></p>
                ...
                <el-collapse class="target-table" @change="handleChange(item)" accordion>
                    <el-collapse-item :title="$t('analyst.industryResearch')">
                    ...
                    <el-collapse-item :title="$t('myTarget.researchReport')">
                        <div class="corpAnalyze-list" v-for="report in item.reportList" :key="report.id">
                            ...
                            <div>
                                <span class="create-right" :class="{'no-report-word': !report.wordUrl}" @click="downPdforWord(report)">【 {{$t('myTarget.downloadWORD')}} 】</span>
                            </div>
                        </div>
                    </el-collapse-item>
        ...
        <el-dialog :title="$t('analyst.industryTracking')" :visible.sync="createTargetShow" width="400px" @close="closeShow">
            <el-form label-width="100px" v-if="!createTargets.id">
                <el-form-item :label="$t('header.industry')"> 
                    <el-select style="width:100%" v-model="createTargetForm.businessId">
                        <el-option
                ...
                <el-form-item :label="$t('analyst.secondaryIndustry')"> 
                    <el-input type="input" v-model="createTargetForm.childBusiness"></el-input>
                </el-form-item> 
                <el-form-item :label="$t('industryModel.author')"> 
                    <el-input type="input" v-model="createTargetForm.author"></el-input>
                </el-form-item>
                <el-form-item :label="$t('dataTable.description')"> 
                    <el-input type="textarea" :rows="3" v-model="createTargetForm.msg"></el-input>
                </el-form-item>
            </el-form>

            <el-form label-width="100px" v-if="createTargets.id">
                <el-form-item :label="$t('dataTable.group')"> 
                    <el-select style="width:100%" v-model="createTargets.groupId">
                        <el-option
...
export default {
    ...
    methods: {
        async queryTarget() {
            ...
            const result = await this.$store.dispatch('AC_TargetBizServiceQueryTargetBiz', params)
            this.targetList = result.data.targetBizs ? result.data.targetBizs : []
            this.loading = false
        },
        ...
        async showTargetDetail(item) {
            if(!item.researchList || item.researchList.length == 0){
                let params = {
                    pageNum: 1,
                    pageSize: 100,
                    corpCode: item.bizCode
                }
                const result = await this.$store.dispatch('AC_CorpAnalyzeServiceQueryCorpAnalyze', params)
                this.$forceUpdate();
                this.$set(item,'researchList',result.data.corpAnalyzes)
            }
        },
        ...
        async addResearch() { 
            ...
            let params = {
                ...
                corpCode: this.selectResearchData.bizCode,
                corpCodeName: this.selectResearchData.bizName,
        ...
        async addTarget() {
            if(this.createTargets.id) {
                this.updateTarget()
                return
            }

            if(!this.createTargetForm.childBusiness || !this.createTargetForm.author || !this.createTargetForm.msg || !this.createTargetForm.businessId){
                this.$message.error(this.$t('message.empty'))
                return
            }

            this.businessList.forEach(item => {
                if(this.createTargetForm.businessId == item.id) {
                    this.createTargetForm.bizCode = item.bizCode
                    this.createTargetForm.bizName = item.name
                }
            })

            if(this.tabSelect != 0) {
                this.createTargetForm.groupId = this.tabSelect
            }
            
            this.createTargetForm.userId = localStorage.getItem('userId')
            this.$store.commit('loadingChange',{show: true,text: this.$t('message.inRequest')})
            const result = await this.$store.dispatch('AC_TargetBizManageAddTargetBizBusiness', this.createTargetForm)
            result.code == 200 ? this.$message.success(this.$t('message.success')) : this.$message.error(this.$t('message.fail') + result.msg)
            this.$store.commit('loadingChange',{show: false})
            this.closeShow()
            this.queryTarget()
        ...
        async updateTarget() {
            if(!this.createTargets.groupId){
                this.$message.error(this.$t('message.empty'))
                return
            }

            this.$store.commit('loadingChange',{show: true,text: this.$t('message.inRequest')})   
            const result = await this.$store.dispatch('AC_TargetBizServiceUpdateTargetBiz', this.createTargets)
        ...
        async uploadReport() {
            ...
            this.uploadReportForm.stockCode = this.selectResearchData.bizCode
            this.uploadReportForm.stockName = this.selectResearchData.bizName
            this.uploadReportForm.targetBizId = this.selectResearchData.id
        ...
        async uploadNewLink() {
            ...
            this.uploadNewLinkForm.stockCode = this.selectResearchData.bizCode
            this.uploadNewLinkForm.stockName = this.selectResearchData.bizName
            this.uploadNewLinkForm.targetBizId = this.selectResearchData.id
